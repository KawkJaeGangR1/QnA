...
3. **Authentication > Users** 탭에서 사용자 추가
   - 이메일: `kawkjaegang@admin.local` (app.js의 `ADMIN_EMAIL`과 반드시 동일해야 함)
...
지금 저장소 구조에 맞춰 이미지는 루트에 바로 두는 방식으로 되어 있습니다
(`gwakjegang_default.png`, `gwakjegang_thinking.png`).
`app.js`의 `EXPRESSION_IMAGES`에 연결되어 있고,
표정을 더 추가하고 싶으면 이미지를 저장소에 올리고
`EXPRESSION_IMAGES`에 한 줄 추가한 뒤 `index.html`의 `.expression-picker` 안에
버튼만 하나 더 넣으면 됩니다.

헤더에 보이는 프로필 사진은 코드 수정 없이, 로그인 후 메뉴 > **프로필 사진 변경**에서
바로 업로드해서 바꿀 수 있습니다. 별도 유료 플랜이 필요한 Storage 대신,
사진을 작게 압축해서 Firestore에 직접 저장하는 방식이라 무료 플랜 그대로 써도 됩니다
(정사각형 기준 240px 정도로 자동 축소됩니다).
